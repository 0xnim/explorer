...
const beautify = (block) => {
    try {
        for (const i in block) {
            if (block[i] instanceof Buffer)
                block[i] = block[i].toString('hex');
            if (i === 'transactions')
                block[i] = block[i].map(transaction => {
                    for (const i in transaction) {
                        if (['to', 'from'].includes(i))
                            transaction[i] = viscoin.Address.toString(transaction[i]);
                        else if (transaction[i] instanceof Buffer)
                            transaction[i] = transaction[i].toString('hex');
                    }
                    return transaction;
                });
        }
